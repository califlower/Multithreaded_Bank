...
void *printAccounts(void *emptyPtr)
{
	while (1)
	{
		...
		int i=0;
		printf("---------------------------------------\n");
		for (i=0;(i<20 && !strcmp(accountList[i].accountName,"")==0);i++)
		{
...
int addAccount(char * name)
{
	...
	{
		int i;
		for (i=0; i<*numAcc && !strcmp(accountList[i].accountName,"")==0; i++)
		{
		...
		strcpy(accountList[*numAcc].accountName,name);
		accountList[*numAcc].balance=0;
		accountList[*numAcc].inUse=0;
...
int startAccount(char * name)
{
	...
	for (i=0; i<maxAcc; i++)
	{
		if (!strcmp(accountList[i].accountName,"")==0)
		{
			return -1;
		}
...
void initConnection()
{
	...
	accountList = mmap(NULL, 20*sizeof(account), PROT_READ | PROT_WRITE, 
